...
notfound_count=0
marker_found = False
marker_location = None  # Store marker GPS location
...
def msg_receiver(message):
    global notfound_count, found_count, time_last, time_to_wait, id_to_find, marker_found, marker_location
        ...
        try:
            if ids is not None:
                if ids[0]==id_to_find:
                    ...
                    # Save marker location when found
                    if not marker_found:
                        marker_found = True
                        marker_location = {
                        ...
                        print('MARKER DETECTED AND LOCATION SAVED!')
                        print('Drone position: Lat=%.6f, Lon=%.6f, Alt=%.2f' % 
                              (marker_location['lat'], marker_location['lon'], marker_location['alt']))
                        print('Marker distance: %.2f cm' % marker_location['distance_cm'])
...
if __name__=='__main__':
    try:
        ...
        if marker_found and marker_location:
            print("\n" + "="*50)
            print("MARKER FOUND - RETURNING TO BASE")
            print("="*50)
            print("Marker Location Report:")
            print("  Latitude:  %.8f" % marker_location['lat'])
            print("  Longitude: %.8f" % marker_location['lon'])
            print("  Altitude:  %.2f m" % marker_location['alt'])
            print("  Distance:  %.2f cm" % marker_location['distance_cm'])
            print("  X Offset:  %.2f cm" % marker_location['x_offset_cm'])
            print("  Y Offset:  %.2f cm" % marker_location['y_offset_cm'])
            print("="*50 + "\n")
            ...
            print("="*50)
            print("ArUco Marker ID 72 was detected at:")
            print("  GPS: %.8f, %.8f" % (marker_location['lat'], marker_location['lon']))
            print("  Altitude: %.2f m" % marker_location['alt'])
            print("  Distance from drone: %.2f cm" % marker_location['distance_cm'])
            print("="*50 + "\n")
