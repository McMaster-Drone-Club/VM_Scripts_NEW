...
def visit_fire_targets(hover_alt=6.0, dwell_time=5.0):
    """
    Visit each detected fire target (all ArUco IDs except 72) and hover above it.
    hover_alt: target altitude in meters
    dwell_time: hover time at each fire in seconds
    """
    global detected_markers

    print("\n" + "="*60)
    print("VISITING ALL FIRE TARGETS AT %.1fm ALTITUDE" % hover_alt)
    print("="*60)

    # Ensure we are in GUIDED mode
    vehicle.mode = VehicleMode('GUIDED')
    while vehicle.mode != 'GUIDED':
        print("Waiting for GUIDED mode before visiting fires...")
        time.sleep(1)
    print("In GUIDED mode. Starting fire inspection flight...")

    # Fire targets = all markers except 72 (person)
    ...
    for idx, mid in enumerate(fire_ids, 1):
        ...
        print("Heading to fire target ID %d..." % mid)
        print("  Target GPS: %.6f, %.6f @ %.2fm" %
              (marker['marker_lat'], marker['marker_lon'], hover_alt))

        vehicle.simple_goto(target)

        # Fly until close to the target and at correct altitude
        while True:
            ...
            if distance < 1.0 and alt_error < 0.5:
                print("  Reached fire target ID %d. Hovering now..." % mid)
                break

            time.sleep(1)

        # Hover over the fire target
        time.sleep(dwell_time)
        print("  Completed hover at fire target ID %d." % mid)

    print("\nFinished visiting all fire targets.")
    print("="*60 + "\n")
