...
def visit_fire_targets(visit_alt=3.0, dwell_time=5.0):
    ...
    - Fly directly to marker GPS at visit_alt (3.0m)
    - Stay there for dwell_time seconds (simulate roof landing)
    - Then go directly to the next fire (also at visit_alt)
    No LAND mode, no NED descent, no disarm/rearm, no takeoff.
    """
    global detected_markers

    print("\n" + "=" * 60)
    print("VISITING FIRE TARGETS: simple_goto at %.1fm for each fire" % visit_alt)
    print("=" * 60)
    ...
    for idx, mid in enumerate(fire_ids, 1):
        ...
        # Target at the desired "roof" altitude
        target = LocationGlobalRelative(
            marker['marker_lat'],
            marker['marker_lon'],
            visit_alt
        )
        print("Heading to fire ID %d at %.1fm altitude (simple_goto)..." %
              (mid, visit_alt))
        ...
        while True:
            ...
            alt_error = abs(current.alt - visit_alt)

            print("  Distance to fire ID %d: %.2f m (alt error: %.2f m)" %
                  (mid, distance, alt_error))

            if distance < 1.0 and alt_error < 0.5:
                print("  Reached fire target ID %d at roof-sim altitude %.1fm." %
                      (mid, visit_alt))
                break
            time.sleep(1)

        print("  Simulating roof landing: waiting %d seconds at fire ID %d..." %
              (dwell_time, mid))
        time.sleep(dwell_time)

    print("\nFinished visiting all fire targets at %.1fm.\n" % visit_alt)
    print("=" * 60)


def return_to_original(target_alt=8.0):
    """Return to original takeoff position at target_alt (e.g. 8m)."""
    global original_position

    print("Returning to original position at %.1fm..." % target_alt)
    ...
    while True:
        ...
        dlong = current.lon - home.lon
        distance = math.sqrt((dlat * dlat) + (dlong * dlong)) * 1.113195e5
        alt_error = abs(current.alt - target_alt)

        print('Distance to original position: %.2f m (alt error: %.2f m)' %
              (distance, alt_error))

        if distance < 1.0 and alt_error < 0.7:
            print('Reached original position at ~%.2fm!' %
                  vehicle.location.global_relative_frame.alt)
            break
        time.sleep(1)

    time.sleep(2)
    print('Return complete. Ready for precision landing.')
            ...
            if ids is not None:
                ...
                for i in range(len(ids)):
                    ...
                    if marker_id not in detected_markers:
                        ...
                        print('  Total unique markers found: %d' %
    ...
    try:
        ...
        if detected_markers:
            ...
            if 72 in detected_markers:
                ...
                print("Starting intermediate step: visiting each detected fire target at 3.0m...")
                visit_fire_targets(visit_alt=3.0, dwell_time=5.0)
                print("Finished visiting all fire targets. Proceeding to return home...")

            print("=" * 60 + "\n")

            # Return to original position at 8m before precision landing
            return_to_original(target_alt=8.0)
