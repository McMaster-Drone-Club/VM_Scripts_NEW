...
# Square search pattern (10x10m area)
# Starting from (0,0), search around (7,7) where house is
search_waypoints = [
    (2, 2),   # Bottom-left corner
    (12, 2),  # Bottom-right corner
    (12, 12), # Top-right corner
    (2, 12),  # Top-left corner
    (7, 7),   # Center (directly over house)
]
...
def get_distance_meters(target_location, current_location):
    """Calculate distance between two LocationGlobal objects in meters"""
...
def goto_position_target_local_ned(north, east, down):
    """
    Send SET_POSITION_TARGET_LOCAL_NED command to move to a location in the North, East, Down frame.
    """
...
def goto_waypoint(north, east):
    """Navigate to waypoint and wait until arrived"""
    print("Going to waypoint: North=%d, East=%d" % (north, east))
    goto_position_target_local_ned(north, east, -search_altitude)
    
    # Wait until drone reaches waypoint (within 1 meter)
    while True:
...
def search_pattern():
    """Execute square search pattern"""
    global current_waypoint_index, marker_found
    
    print("Starting square search pattern...")
    
    while current_waypoint_index < len(search_waypoints) and not marker_found:
        north, east = search_waypoints[current_waypoint_index]
        goto_waypoint(north, east)
        
        # Hover for 3 seconds to scan for marker
        print("Hovering to scan for marker...")
        time.sleep(3)
    ...
    if not marker_found:
        print("Search complete - marker not found. Returning to launch...")
...
def msg_receiver(message):
    ...
    if time.time() - time_last > time_to_wait:
        ...
        try:
            if ids is not None:
                if ids[0]==id_to_find:
                    marker_found = True  # Signal that we found the marker
...
if __name__=='__main__':
    try:
        ...
        # If marker was found during search, start precision landing subscriber
        if marker_found:
            subscriber()
